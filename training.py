...
def train(epoch):
    ...
    for batch_idx, (unlabeled_inputs, _) in enumerate(train_unlabeled_loader):
        ...
        # 监督损失
        sup_loss1 = ce_loss(logits_outputs[-1], targets, reduction='mean') # 合并
        sup_loss2 = criterion(logits_outputs[0],targets) # beginmodel
        sup_loss3 = criterion(logits_outputs[-2],targets) # VAE
        ...
        # unsup_loss2 = (ce_loss(logits_unlabeled_outputs[0], pseudo_labels2) * weights2).mean()      
        loss_VAE = net.model4.loss_function(outputs[2],M_N=0.001)['loss'] # recons VAE
        loss = sup_loss1 + sup_loss2 + sup_loss3 + unsup_loss1 + loss_VAE
        loss.backward()
        optimizer.step()
        net.zero_grad()
        ...
        if batch_idx % len(trainloader)*2000 == 0:
            logger.info(f'sup_loss1: {sup_loss1}  loss_VAE: {loss_VAE}  sup_loss2: {sup_loss2}  unsup_loss1: {unsup_loss1} ')
            # logger.info(f'sup_loss1: {sup_loss1}  loss_VAE: {loss_VAE} unsup_loss1: {unsup_loss1}')
            # logger.info(f'sup_loss2: {sup_loss2} unsup_loss2: {unsup_loss2}')
    # scheduler.step()
...
def test(epoch):
    ...
    with torch.no_grad():
        ...
        if _F1 > best_eval_F1:
            ...
            torch.save(state, '/aidata/Ly61/number5/CL0322/ckpt/ckpt_{}_{}.pth'.format(args.Dataset,args.version))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch-size', type=int, default=512, help='Input batch size for training (default: 32)')
    ...
    # model
    parser.add_argument("--version", type=str, default='0416', help="")
    ...
    parser.add_argument("--vae_number", type=int, default=30000, help="")
