...
def train(epoch):
    ...
    for batch_idx, data  in enumerate(combined_trainloader):
        ...
        # with torch.no_grad():
        #     outputs_old = net2(profiles, dm_curves, subbands, subints)
            
        optimizer.zero_grad()
        outputs = net(profiles, dm_curves, subbands, subints,'task2')  
        kl = net.get_kl()
        loss = elbo(outputs, targets, kl)
        # loss_distill = distillation_loss(outputs, outputs_old,T=Tem)
        ...
        if batch_idx % (len(trainloader)*5) == 0:
            ...
            # logger.info(f'loss: {loss.item():.6f} kl_loss: {kl_loss} loss_new: {loss_new.item():.6f} loss_old: {loss_old:.6f} loss_distill: {loss_distill:.6f}')
            logger.info(f'loss: {loss.item():.10f}')
...
def test(epoch):
    ...
    with torch.no_grad():
        ...
        # Save checkpoint.
        if _F1 > best_eval_F1 and _F1 > 0.90:
            state = {
            'net': net.state_dict(),
            'F1': _F1,
            'epoch': epoch,
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--version', type=str, default='050701', help=' Version')
    parser.add_argument('--version', type=str, default='080501', help=' Version')
    ...
    # 配置日志记录器
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', filename='/aidata/Ly61/number5/CL0322/{}_CL_{}.log'.format(args.Dataset,args.version), filemode='a')
    ...
    # args.train_unpulsar = 300
    # args.train_pulsar = 300
    ...
    # args.train_unpulsar = 600
    # args.train_pulsar = 600
    ...
    # combined_trainloader = trainloader2
    ...
    oor = torch.load('/aidata/Ly61/number5/CL0322/ckpt/FAST/080501/ckpt_best.pth')
    ...
    # layers_to_unfreeze1 = ['conv2','dropout','fc1']
    # layers_to_unfreeze2 = ['pre','stage1']
    # layers_to_unfreeze = ['decoder']
